...
RC CreateUniqueIndexExecutor::execute(SQLStageEvent *sql_event)
{
  ...
  CreateUniqueIndexStmt *create_index_stmt = static_cast<CreateUniqueIndexStmt *>(stmt);
  
  Trx *trx = session->current_trx();
  Table *table = create_index_stmt->table();
  return table->create_unique_index(trx, create_index_stmt->field_metas(), create_index_stmt->index_name().c_str());
}